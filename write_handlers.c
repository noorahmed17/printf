...
extra_ch = ' ';
return (write_num(ind, buffer, flags, wi, pre,
...
buffer[i] = padd;
buffer[i] = '\0';
if (fla & F_MINUS && padd == ' ')/* Asign extra char to left of buffer */
{
if (extra_c)
buffer[--ind] = extra_c;
return (write(1, &buffer[ind], length) + write(1, &buffer[1], i - 1));
}
else if (!(fla & F_MINUS) && padd == ' ')/* extra char to left of buff */
{
if (extra_c)
buffer[--ind] = extra_c;
return (write(1, &buffer[1], i - 1) + write(1, &buffer[ind], length));
}
else if (!(fla & F_MINUS) && padd == '0')/* extra char to left of padd */
{
if (extra_c)
...
if (extra_c)
buffer[--ind] = extra_c;
return (write(1, &buffer[ind], leth) + write(1, &buffer[3], i - 3));
}
...
if (extra_c)
buffer[--ind] = extra_c;
return (write(1, &buffer[3], i - 3) + write(1, &buffer[ind], leth));
}
